...
class RelationshipVis {
    ...
    updateVisInterests(list_selectedAreas){
        let vis = this;
        ...
        vis.tempLinks_areasFaculty1 = vis.data_areas.filter(x => vis.list_faculty1.includes(x.Title)
            & vis.nodes_areasInterests.filter(x => (x.hasLinks === true)).map(x => x.name).includes(x.Area))
            .map(x => ({
                'source': vis.nodes_faculty1.filter(y => {return y.name === x.Title})[0],
                'target': vis.nodes_areasInterests.filter(y => {return y.name === x.Area})[0],
                'isArea': true}))
        vis.tempLinks_interestFaculty1 = vis.data_interests.filter(x => vis.list_faculty1.includes(x.Title)
            & vis.nodes_areasInterests.filter(x => (x.hasLinks === true)).map(x => x.name).includes(x.Interest))
            .map(x => ({
                'source': vis.nodes_faculty1.filter(y => {return y.name === x.Title})[0],
                'target': vis.nodes_areasInterests.filter(y => {return y.name === x.Interest})[0],
                'isArea': false}))
        ...
        vis.links_areasInterestsFaculty1.forEach(function(li){
            vis.svg.append("path", "g")
                .attr("class", "link")
                //.attr("id", li.id)
                .attr("d", function () {
